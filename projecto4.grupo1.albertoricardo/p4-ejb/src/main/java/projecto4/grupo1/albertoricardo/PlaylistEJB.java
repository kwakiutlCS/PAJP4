...
public class PlaylistEJB implements PlaylistEJBLocal {
    ...
    public List<PlaylistEntity> getOwnPlaylists(int id) {
        List<PlaylistEntity> pe = new ArrayList<>();
        try {
            Query q = em.createQuery("SELECT p FROM PlaylistEntity p WHERE p.userOwner.id = :id").setParameter("id", id);
            pe = q.getResultList();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ListPlaylists getPlaylistsFromUser(Object id) {
        Query q = em.createQuery("select p from PlaylistEntity p where p.userOwner.id = :i").setParameter("i", id);
        @SuppressWarnings("unchecked")
        List<PlaylistEntity> lpe = q.getResultList();
