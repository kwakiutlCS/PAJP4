...
public class Users {
    ...
    public Response userDetails(@PathParam("id") int id) {
        ...
        if (ud != null) {
            ...
            return Response.status(Response.Status.NOT_FOUND).entity("Utilizador não encontrado com o id: " + id).type(MediaType.TEXT_PLAIN).build();
    }
    ...
    public Response changePassword(@FormParam("id") String id, @FormParam("password") String password) {
        ...
        if (ud != null) {
            if (userejb.changePassword(ud, password))
                return Response.status(Response.Status.OK).entity("Password do utilizador com o id " + id + " alterada com sucesso").type(MediaType.TEXT_PLAIN)
                        .build();
            else
                return Response.notModified().build();
        } else
            return Response.status(Response.Status.NOT_FOUND).entity("Utilizador com o id " + id + " nao existe").type(MediaType.TEXT_PLAIN).build();
    }
