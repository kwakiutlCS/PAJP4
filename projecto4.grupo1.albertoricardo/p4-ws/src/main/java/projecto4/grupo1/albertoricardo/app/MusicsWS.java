...
public class MusicsWS {
    ...
    public Response getAllMusics() {
        ...
        if (lme != null) {
            ...
            return Response.status(Response.Status.NO_CONTENT).entity("Nenhuma musica encontrada.").type(MediaType.TEXT_PLAIN).build();
    }
    ...
    public Response getMusicsFromUser(@PathParam("id") int id) {
        ...
        if (lme != null) {
            ...
            return Response.status(Response.Status.NOT_FOUND).entity("Nenhuma musica encontrada do utilizador com id " + id).type(MediaType.TEXT_PLAIN).build();
    }
